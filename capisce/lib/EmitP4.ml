...
module Parser = struct
  ...
  type transition = 
    { 
      discriminee : Expr.t option;
      cases : (Expr.t * state_name) list;
      default : state_name
    }
  ...
  let vars (prsr : t) : Var.t list =
    let transition_vars {discriminee;cases; default=_} =
      ...
      let discriminee_vars =
        Option.value_map discriminee ~f:(concat Expr.vars) ~default:[]
      in 
      let cases_vars = 
        List.bind cases ~f:(fun (e, _) -> concat Expr.vars e)
      in
  ...
  let to_gcl unroll (parser : t) : GCL.t =
    ...
    and inline_transition unroll_map {discriminee;cases;default} =
      ...
      | Some query -> 
        ...
        ~f:(fun else_ (test, next_state) -> 
          let then_ = inline_next_state next_state in 
          GCL.ite (BExpr.eq_ query test) 
            then_ 
            else_
        )
    in
  ...
  let emit_parser_state {name; body; transition} : string = 
    ...
    let emit_transition transition =
      ...
      | Some query -> 
        List.map transition.cases ~f:(fun (expr, state) -> 
          Printf.sprintf "      %s : %s;" (Expr.emit_p4 expr) state
        )
        |> String.concat ~sep:"\n"
  ...
  let table_definitions =
    ...
    let keys = 
      List.map table.keys ~f:(fun (key, kind) ->
        Printf.sprintf "\n      %s : %s;" (Var.str key) (string_of_kind kind)
      )
      |> String.concat
    in
    let actions = 
...
let emit_metadata psm ing eg =
  let all_vars =
    ...
    |> List.dedup_and_sort ~compare:Var.compare
  in 
  let metadata = Metadata.get_metadata "my_metadata_t" all_vars in
