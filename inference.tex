  ...
  \item[$(c = t.\apply())$] Let $\pkt$ and $\sigma$ be arbitrary. Let
    ...
    \item[\textit{Case 1}.] Assume that there is some $1 \leq i \leq n$ such
      ...
      \[\bigcup_{a=0}^{|t.\action| - 1}\denote{t.\action[a](\rho_t.\data)}^\dagger H_a
       ...
       Lemma~\ref{lem:proj-den-comm}, since $H_{r_i.\action} =_v \{\pkt\}$. The result follows by the IH on
       $t.\action[r_i.\action](r_i.\data)$ and Corollary~\ref{cor:equal-proj}.
...
\begin{figure}[H]
  \[
    \begin{array}{>{\displaystyle}c}
      ...
      \infrule{\psi[\forall x. x = e]}{\FALSE}{UnivEq}
      \qquad
      \infrule{\psi[\exists x. x = e]}{\TRUE}{ExtEq}
    \end{array}
  \]
\end{figure}
...
\begin{proof}
  ...
  \begin{enumerate}[align=left]
    ...
    \item[(\textsc{UnusedUVar})] \todo[inline]{Prove this case}
    \item[(\textsc{UnusedEVar})] \todo[inline]{Prove this case}
    \item[(\textsc{UnivEq})] \todo[inline]{prove this case}
    \item[(\textsc{ExtEq})] \todo[inline]{prove this case}
