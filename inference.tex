...
\begin{figure}[htp]
  ...
  \multicolumn{3}{l}{\denote{c}^\sigma : \Model \to \Pkt \to \mathcal{P}(\Model \times \Pkt)_\error }\\
    ...
    \end{cases} \\
    \denote{\choose \rho t}^\sigma~\mu~\pkt &&
    \bigcup_{r \in \sigma(t)}\{(\pkt,\mu\{\rho \mapsto r\})\}
    \\
    ...
    \begin{cases}
      ...
      & \text{where}~r = \argmin_i\{\pkt,\mu \models_\sigma \sigma(t.\id)[i].\matches = t.\keys \} \\
...
\begin{proof}
  ...
  \item[$(c = t.\apply())$]
    ...
    \item[\textsc{Hit}] In this case, there is some $r_i$ such that $\pkt,\mu
      ...
      Now by the sequential composition rule, we can determine the result of the
      first branch by computing as follows:
      \[\begin{array}{>{\displaystyle}ll}
      \denote{\bigchoice_{i=0}^{|t.\action| -1}
        \left\{\begin{array}{l}
        \assume \rho.\action = i;\\
        t.\actions[i](\rho.\data)
        \end{array}\right\}
      }^\sigma~(\mu\{\rho \mapsto r_i\})~\pkt \\
      = \denote{\denote{c[r_i.\data/\vec x]}^\sigma~(\mu\{\rho \mapsto r\})~\pkt}\\
      & \textrm{skipping lots of definition steps} \\
      \end{array}
      \]
      \todo[inline]{Need Scoping Rules for $\rho$}
