...
\begin{figure}
  ...
  \todo[inline]{@Nate I know you wanted a standard satisfaction relation, but this was easier to think about in the context of the domain. The difference boils down to a naming convention}
...
\section{P4v comparison}

Ive said before that the P4v-style instrumentation is an overapproximation of
...
\begin{figure}
  \[\begin{array}{l >{\triangleq}cl}
  \multicolumn{3}{l}{\pv : \Cmd \to \Cmd} \\
  \multicolumn{3}{l}{\pv : \Prog \to \Instr} \\
  \pv(h.f := e) && h.f := e \\
  \pv(\assume b) && \assume b \\
  \pv(\assert b) && \assert b \\
  \pv(c_1;c_2) && \pv(c_1;c_2) \\
  \pv(c_1 \choice c_2) && \pv(c_1) \choice \pv(c_2) \\
  \pv(\choose \rho t c) && \textcolor{red}{\text{undefined}} \\
  \pv(t.\apply()) && \\
...
\begin{theorem}[\texttt{p4v} over-approximates]
  ...
  $\denote{c}^\sigma\,\mu\,\pkt \subseteq \denote{\pv(c)}^\sigma\,\mu\,\pkt$
\end{theorem}
...
Whereas
\[\begin{array}{l}
\denote{\pv(t.\apply())}^\emptyset\,\emptyset\,\pkt \\
= \denote{
    \displaystyle
    \bigchoice_{i = 0}^{|t.\action|-1} t.\action[i]()
