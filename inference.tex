...
\end{lemma}
\begin{proof}
  Let $c_1, c_2$, $\sigma$, and $\pkt$ be given as above.
  Let $\{i,j\} = \{1,2\}$. Observe:
  \[\begin{array}{ll}
  \denote{c_i;c_j}^\sigma \pkt
  = {\denote{c_j}\sigma}^\dagger\left(\denote{c_i}^\sigma \pkt\right)
  \end{array}\]

  Let $V = \Var \setminus \fvs(c_1;c_2)$. Let $P_1 = \denote{c_1}^\sigma \pkt$,
  and $P_2 = \denote{c_2}^\sigma \pkt$. By Lemma~\ref{lem:parametricity-set}, the following hold
  \[P_1 =_{\fvs(c_2) \cup V} \{\pkt\}\] \[P_2 =_{\fvs(c_1) \cup V} \{\pkt\}\]
  So, Let $P_{12} = {\denote{c_2}^\sigma}^\dagger P_1$, and $P_{21} =
  {\denote{c_1}^\sigma}^\dagger P_2$. Again, by
  Lemma~\ref{lem:parametricity-set}, the following equivalences hold
  \[P_{12} =_{\fvs(c_1) \cup V} P_1\] \[P_{21} =_{\fvs(c_2) \cup V} P_2\]
  Now by Lemma~\ref{lem:proj-set-subset}, we know $P_{12} =_V \{\pkt\} = P_{21}$.

  Now by Lemma~\ref{lem:param-noninter-set} we know
  \[P_{21} =_{\fvs(c_1)} P_1\] \[P_{12} = \fvs(c_2) P_2\]

  Then Lemma~\ref{lem:proj-set-subset} gives
  \[P_{12} =_{\fvs(c_1)} {P_1}\] \[P_{21} =_{\fvs(c_2)} {P_2}\]

  Finally Lemma~\ref{lem:union} proves our goal:
  \[P_{12} =_{\fvs(c_1) \cup \fvs(c_2) \cup V} P_{21}\]
