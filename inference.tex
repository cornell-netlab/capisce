...
\begin{figure}[htp]
  \[\begin{array}{lclll}
    ...
    e & ::= & v & \textit{Value} \\
      ...
      & \mid & x & \textit{Action Data Variable}\\
...
\begin{conjecture}
  For every $c \in \Cmd$, and $\sigma \in \State$, the following formula is valid
  \[\state\sigma_c \Rightarrow  \WP(c\instr,\TRUE) \iff \WP(c[\sigma], \TRUE)\]
\end{conjecture}
...
\begin{quote}
  \textbf{Conservative Problem.} Given a command $c \in \Cmd$, compute (the
  weakest) control plane formula $\varphi$ such that if, for a state $\sigma :
  \Value \to \Row^*$, the following judgement is valid \[\state\sigma_c
  \Rightarrow \varphi,\] then \[\forall \pkt \in \Pkt.~ \denote{c}^\sigma
  \emptyset\,\pkt \neq \error \]
\end{quote}

However, this $\phi$ may be $\FALSE$ for many realistic programs $c$, so we may
prefer to solve the following problem:

\begin{quote}
  \textbf{Flexible Problem.} Given a command $c \in \Cmd$, compute (the weakest)
  ...
  \[b \wedge \state\sigma_c \Rightarrow \varphi,\]
...
\item[\textit{Nondeterminism} ($c_1 \choice c_2$).] To solve the \textbf{Conservative
