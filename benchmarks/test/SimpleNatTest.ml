...
let simple_nat fixed =
  pipeline simple_nat_parser (simple_nat_ingress fixed) simple_nat_egress
  |> HoareNet.assert_valids


let test_annotations () =
  HoareNet.check_annotations (simple_nat true)
  |> Alcotest.(check bool) "check_annotations should pass" true

let test_infer_timeout () =
  HoareNet.infer ~qe:`Enum (simple_nat true) None None
  |> Alcotest.(check @@ neg Equivalences.smt_equiv)
    "Enum CPI is not trivial"
    BExpr.true_

let test_concolic () =
  HoareNet.infer ~qe:`Conc (simple_nat true) None None
  |> Alcotest.(check @@ neg Equivalences.smt_equiv)
