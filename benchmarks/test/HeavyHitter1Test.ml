...
let hh_ingress =
  ...
  in
  let count_table =
    let symb_count_match = Var.make "_symb$count_table$match_0" 32 in
    let symb_count_DONTCARE = Var.make "_symb$count_table$match_0$DONT_CARE" 1 in
    sequence [
      ifte_seq (eq_ btrue @@ var symb_count_DONTCARE) [
      ] [
        assert_ @@ eq_ btrue @@ var hdr.ipv4.isValid;
        assume @@ eq_ (var symb_count_match) (var hdr.ipv4.srcAddr);
      ];
