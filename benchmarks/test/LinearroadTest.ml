...
let linearroad_ingress _ (*annotated*) =
  ...
  let set_spd =
    [], Action.[
        assign meta.seg_meta.ewma_spd @@ bslice 0 15 @@ var hdr.pos_report.spd;
        assert_ @@ eq_ btrue @@ var hdr.pos_report.isValid;
  ...
  let calc_ewma_spd =
    [], Action.[
        ...
        lshr_ (bmul (bslice 0 15 @@ var hdr.pos_report.spd) (bvi 32 16)) (bvi 7 16)
        |> (bslice 0 31)
        |> badd (bmul (bslice 0 31 @@ var meta.seg_meta.ewma_spd) (bvi 96 32))
        |> bslice 0 15
  ...
  let issue_toll =
    ...
    [base_toll], Action.[
        ...
        (bsub (bslice 0 15 @@ var meta.seg_meta.vol) (bvi 50 16))
        |> bmul (bsub (bslice 0 15 @@ var meta.seg_meta.vol) (bvi 50 16))
        |> bmul (var base_toll)
        |> assign toll_meta.toll;
        assert_ @@ eq_ btrue @@ var hdr.pos_report.isValid;
        (* v_accnt_bal_reg.read(meta.toll_meta.bal, (bit<32>)hdr.pos_report.vid); *)
        assign meta.toll_meta.bal @@ badd (var meta.toll_meta.bal) @@ bslice 0 31 @@ var meta.toll_meta.toll;
        assert_ @@ eq_ btrue @@ var hdr.pos_report.isValid;
  ...
  ifte_seq (eq_ btrue @@ var hdr.ipv4.isValid) [
    ifte_seq (eq_ btrue @@ var hdr.pos_report.isValid) [
      ...
      ifte (ands_[
          ...
          eq_ (var hdr.pos_report.dir) (bslice 0 7 @@ var meta.v_state.prev_dir);
          eq_ (var hdr.pos_report.lane) (bslice 0 7 @@ var meta.v_state.prev_lane);
        ])
        loc_not_changed
        loc_changed;
      ifte (ands_ [
...
let test_infer_buggy () =
  Printf.printf "GPL Program: %s ------\n" @@ HoareNet.to_string (linearroad false);
  Alcotest.check_raises
    "Enum CPI is unsolveable"
    (Failure "unsolveable")
    (fun () -> ignore (HoareNet.infer ~qe:`Enum (linearroad false) None None))
...
let test_concolic_buggy () =
  Alcotest.check_raises
    ...
    (Failure "unsolveable")
    (fun () -> ignore (HoareNet.infer ~qe:`Conc (linearroad false) None None))
...
let tests : unit Alcotest.test_case list = [
  ...
  Alcotest.test_case "Linearroad enum unannotated" `Slow test_infer_buggy;
  Alcotest.test_case "Linearroad infer enum annotated" `Slow test_infer_fixed;
  Alcotest.test_case "Linearroad infer conc unannotated" `Quick test_concolic_buggy;
  Alcotest.test_case "Linearroad infer conc fixed" `Quick test_concolic_fixed;
]
